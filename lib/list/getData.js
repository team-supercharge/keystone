		...
		fields.forEach(function (path) {
			...
			if (field) {
				if (field.type === 'relationship' && expandRelationshipFields) {
					...
					// ================================
					// PLEASE NOTE: Had to put this conditional in, because of the list view of log revisions
					// In case -mostly on previous revision entries- the revokedAt (time of edit) is not stored.
					// The expected behaviour is to not show any date in this case, so need to send back empty string for the frontend.
					// ================================
					data.fields[path] = path === 'revokedAt' && field.getData(item) === undefined ? '' : field.getData(item);
				}
			} else {
