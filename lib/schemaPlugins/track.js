...
module.exports = function track () {
	...
	const showMeta = !!list.get('showMeta');
		...
		if (_.has(defaultOptions, key)) {
			...
			if (value) {
				...
				switch (key) {
					case 'createdAt':
					case 'updatedAt':
						fields[fieldName] = { type: Date, noedit: true, collapse: true,
							index: true, hidden: !showMeta };
						break;

					case 'createdBy':
					case 'updatedBy':
						fields[fieldName] = { type: Types.Relationship, ref: userModel,
							noedit: true, collapse: true, index: true, hidden: !showMeta };
						break;
				}
			}
		} else {
